<?php
/*
 $Id:   aupost.php,v2.5.5.05 Sep 2023
        V2.5.5.05  // update ln40 as well
...
if (!defined('VERSION_AU')) { define('VERSION_AU', '2.5.5.05');}

// +++++++++++++++++++++++++++++
define('AUPOST_MODE','Test'); //Test OR PROD    // Test uses test URL and Test Authkey;
...
class aupost extends base
{
    ...
    public function quote($method = '')
    {
       ...
       // BMH not developed $FlatText = " Using AusPost Flat Rate." ; // BMH Not in use
        ...
        for($x = 0 ; $x < count($myorder) ; $x++ ) {
           ...
           if (MODULE_SHIPPING_AUPOST_DEBUG == "Yes" ) {
                ...
                echo "<center><table class=\"aupost-debug-table\" border=1><th colspan=8> Debugging information ln343 [aupost Flag set in Admin console | shipping | aupost]</hr>
        ...
        */
        ...
        if (MODULE_SHIPPING_AUPOST_DEBUG == "Yes" && BMHDEBUG1 == "Yes") {
            echo '<p class="aupost-debug"> <br>parcels ***<br>aupost ln775 ' .'https://' . $aupost_url_string . PARCEL_URL_STRING . MODULE_SHIPPING_AUPOST_SPCODE . "&to_postcode=$dcode&length=$parcellength&width=$parcelwidth&height=$parcelheight&weight=$parcelweight" . '</p>';
        }
        //// ++++++++++++++++++++++++++++++
        // get parcel api';
        ...
        if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes" ) && (BMHDEBUG2 == "Yes")) {
            echo "<table class='aupost-debug'><tr><td><b>auPost - Server Returned BMHDEBUG2 ln784:</b><br>" . $qu . "</td></tr></table> " ;
        }  //eof debug

        // Check for returned quote is really an error message
        //
            if(str_starts_with($qu, "{" )) {
            $myerrorarray=json_decode($qu); echo '<br> ln792 ';  //BMH
            print_r($myerrorarray);
            echo '<br> ln794 myerrorarray[status] = ' . $myerrorarray[status];
            $myerrorarray=json_decode($qu);
            echo '<br> ln796 $myerrorarray ='; print_r($myerrorarray);
        ...
        if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes" ) && (BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
            echo "<p class='aupost-debug1' ><strong> >> Server Returned BMHDEBUG1+2 line 809 << <br> </strong> <textarea  > ";
        ...
        $i = 0 ;  // counter
        if (( MODULE_SHIPPING_AUPOST_DEBUG == "Yes" ) && (BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes"))  {
            echo '<br> ln 819 $this->allowed_methods = '; var_dump($this->allowed_methods); // BMH ** DEBUG
        }
        ...
        foreach($xml as $foo => $bar) {
            ...
            if (( MODULE_SHIPPING_AUPOST_DEBUG == "Yes" ) && (BMHDEBUG1 == "Yes"))  {
                ...
                echo "ln 834 ID= $id  DESC= $description COST= $cost inc" ;
                echo "</td></tr></table>" ;
            ...
            switch ($id) {
                ...
                //case  "AUSPARCELREGULARSATCHEL500G" ;       // fall through and treat as one block
                    ...
                    if (in_array("Prepaid Satchel", $this->allowed_methods,$strict = true)) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln849 allowed option = prepaid satchel';
                        }
                ...
                case  "AUSPARCELEXPRESSSATCHELSMALL" ;      // fall through and treat as one block
                    ...
                    if ((in_array("Prepaid Express Satchel", $this->allowed_methods))) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln944 allowed option = parcel express satchel';
                        }
                    ...
                    if ( in_array("Prepaid Express Satchel Insured +sig", $this->allowed_methods) ) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln967 allowed option = parcel express satchel ins+sig'; }
                       if ($ordervalue > $MINVALUEEXTRACOVER) {
                            $optioncode = 'AUS_SERVICE_OPTION_SIGNATURE_ON_DELIVERY';
                            $optionservicecode = ($xml->service[$i]->code);  // get api code for this option
                            $suboptioncode = 'AUS_SERVICE_OPTION_EXTRA_COVER';
                ...
                case  "AUSPARCELREGULARPACKAGE";            // normal mail - own packaging
                    if (in_array("Regular Parcel", $this->allowed_methods,$strict = true)) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln995 allowed option = parcel regular';
                        }
                        $optioncode =""; $optionservicecode = ""; $suboptioncode = ""; $allowed_option ="";
                    ...
                    if ( in_array("Regular Parcel Insured +sig", $this->allowed_methods) ) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln1014 allowed option = parcel regular ins + sig';
                        }
                        if ($ordervalue > $MINVALUEEXTRACOVER) {
                            $optioncode = 'AUS_SERVICE_OPTION_SIGNATURE_ON_DELIVERY';
                            $optionservicecode = ($xml->service[$i]->code);  // get api code for this option
                    ...
                    if ( in_array("Regular Parcel +sig", $this->allowed_methods) ) {
                       if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln1030 allowed option = parcel regular + sig';
                        }
                        $optioncode = 'AUS_SERVICE_OPTION_SIGNATURE_ON_DELIVERY';
                        $optionservicecode = ($xml->service[$i]->code);  // get api code for this option
                ...
                case  "AUSPARCELEXPRESS" ;              // express mail - own packaging
                    if (in_array("Express Parcel", $this->allowed_methods,$strict = true)) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln1117 allowed option = parcel express';
                        }
                        $optioncode =""; $optionservicecode = ""; $suboptioncode = ""; $allowed_option ="";
                    ...
                    if ( in_array("Express Parcel Insured +sig", $this->allowed_methods, $strict = true) ) {
                        if ((BMHDEBUG1 == "Yes") && (BMHDEBUG2 == "Yes")) {
                            echo '<br> ln1152 allowed option = parcel express ins + sig';
                        }
                        if ($ordervalue > $MINVALUEEXTRACOVER) {
                            $optioncode = 'AUS_SERVICE_OPTION_SIGNATURE_ON_DELIVERY';
                            $add = MODULE_SHIPPING_AUPOST_EXP_HANDLING ;
...
// // // BMH _get_secondary_options
    ...
    // PARCELS
        ...
        $db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) VALUES ('Tax Class', 'MODULE_SHIPPING_AUPOST_TAX_CLASS', '1', 'Set Tax class or -none- if not registered for GST.', '6', '60', 'zen_get_tax_class_title', 'zen_cfg_pull_down_tax_classes(', now())");
...
// // // extra functions
    ...
    if (BMHDEBUG2 == "Yes") {
        echo '<p class="aupost-debug"> <br> ln1484 $ret= ' . $ret .  '</p> ' ;// . '<br> var_dump = '; var_dump($ret);
        //$myarray=json_decode($ret);
       // echo '<br> ln1484 $myarray= '; print_r($myarray); echo '<br> '; var_dump($ret);
       // echo ' /p>';

    }
    //BMH 2023-01-23 added code for when Australia Post is down //BMH bof
    $edata = curl_exec($crl);   //  echo '<br> ln1487 $edata= ' . $edata; //BMH DEBUG
