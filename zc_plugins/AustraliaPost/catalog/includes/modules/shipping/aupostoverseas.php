...
/*
 * $Id:   overseasaupost.php, v3.1 September 2026
 * v3.0.0 2026-04-17 v3.0.0 - Conversion from XML API to JSON API
 *  check for additional fee is zero; regrouped  AuPost parcel type description codes that keep changing;
 *  css for debugging pulled in from encapsulated folder via observer auto.aupost_css_loader.php
 * v3.0.1 issue #48 ln1475
 * v3.1 duplicate code moved to functions; check and recover for invalid destination country;
 *      normalise: single service returned as object rather than array for when only one option is returned
 */

if (!defined('VERSION_AU_INT')) {
    define('VERSION_AU_INT', '3.1');
}
...
define('BMHDEBUG_INT3', 'No');           // BMH 3rd level debug to display all returned data from Aus Post
define('USE_CACHE_INT', 'Yes');           // BMH disable cache // set to 'No' for testing;
define('MINEXTRACOVER_OVERIDE', 'No');   // BMH obtain cost for extra cover even if $ordervalue < $MINVALUEEXTRACOVER_INT for testing. For PRODUCTION SET TO "No'

//BMH declare constants

if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_HIDE_PARCEL')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_HIDE_PARCEL', '');
} //
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_TAX_CLASS')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_TAX_CLASS', '');
}
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_TYPES1')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_TYPES1', '');
}
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_STATUS')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_STATUS', '');
}
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_SORT_ORDER')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_SORT_ORDER', '');
}
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_ICONS')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_ICONS', '');
}
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_CORE_WEIGHT')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_CORE_WEIGHT', '');
}
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_TAX_BASIS')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_TAX_BASIS', 'Shipping');
}

// ++++++++++++++++++++++++++
if (!defined('MODULE_SHIPPING_OVERSEASAUPOST_AUTHKEY')) {
    define('MODULE_SHIPPING_OVERSEASAUPOST_AUTHKEY', '');
}
...
if (!defined('PARCEL_INT_URL_STRING')) {
    define('PARCEL_INT_URL_STRING', '/postage/parcel/international/service.json?');
}    // Aust Post URI api what services are avail for destination
if (!defined('PARCEL_INT_URL_STRING_CALC')) {
    define('PARCEL_INT_URL_STRING_CALC', '/postage/parcel/international/calculate.json?');
}   // Aust Post URI api calc charges for each type
...
class aupostoverseas extends base
{
    ...
    public ?string $dcode;              // destination country 2 letter code

    public ?array $dims;               //
    public ?bool $enabled;            // Shipping module status
    public ?string $error_msg_ap_int;   //
    public ?string $icon;               // Shipping module icon filename/path
    public ?string $included_option;    //
    ...
    public ?string $tax_class_int;      //
    public ?string $tax_class;      //

    public ?string $title;              // Shipping module display name
    public ?string $usemod;             //
    public ?bool $usetitle;           //
    public ?int$_check;             //

    public $jsonint = [];           // json array

    private const GATE_NONE = 0;         // no minimum order-value gate
    private const GATE_DOWNGRADE = 1;    // below min extra cover: drop back to signature only
    private const GATE_SKIP = 2;         // below min extra cover: skip the option
    private const DUMP_NONE = 0;         // do not dump the secondary option result
    private const DUMP_VARDUMP = 1;      // dump the result with var_dump (legacy style)
    private const DUMP_DEBUGOUTPUT = 2;  // dump the result with _debug_output
    ...
    public function __construct()
    {
        ...
        $this->tax_class_int = defined('MODULE_SHIPPING_OVERSEASAUPOST_TAX_CLASS') ? MODULE_SHIPPING_OVERSEASAUPOST_TAX_CLASS : null;   // $this->tax_basis = 'Shipping' ;

        $this->tax_basis = defined('MODULE_SHIPPING_OVERSEASAUPOST_TAX_BASIS') ? MODULE_SHIPPING_OVERSEASAUPOST_TAX_BASIS : null;   // disable only when entire cart is free shipping
    ...
    public function quote($method = '')
    {
        ...
        if (zen_not_null($method) && (isset($_SESSION['overseasaupostQuotes']))) {
            ...
            if ($this->tax_class_int > 0) {
                $this->quotes['tax'] = zen_get_tax_rate((int) $this->tax_class_int, $order->delivery['country']['id'], $order->delivery['zone_id']);
            }
            if (zen_not_null($this->icon))
                $this->quotes['icon'] = zen_image($this->icon, $this->title); // set icon for  quotes array

            if (BMHDEBUG_INT1 == "Yes" && BMHDEBUG_INT2 == "Yes") {
                $this->_debug_output("x", 'ln' . __LINE__ . ' x2 aupost-overseas  return->quotes<br>', $this->quotes);
            }
        ...
        // ---- default dimensions for parcels ---------------------------- //
        ...
        $dest_country_name = ($order->delivery['country'] ?? '');      //
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes")) {
            $this->_debug_output("n", 'ln' . __LINE__ . ' n1 AUOS $dest_country code = ' . $dest_country, "");
        }
        ...
        if ($aus_rate_int == 0) {                                       // included by to avoid possible divide  by zero error
        ...
        // ---- Do Not proceed for AU addresses ---------------------------- //
        if ($dest_country == "AU") {
        ...
        for ($x = 0; $x < count($myorder); $x++) {
            ...
            $var = array($dims->fields['products_width'], $dims->fields['products_height'], $dims->fields['products_length']);
            sort($var);
            $dims->fields['products_length'] = $var[2];
            $dims->fields['products_width'] = $var[1];
            $dims->fields['products_height'] = $var[0];
        ...
        if (USE_CACHE_INT == "Yes") {   //BMH DEBUG disable cache for testing
            if (isset($_SESSION['overseasaupostParcel'])) {
                ...
                if (
                    ...
                    if (BMHDEBUG_INT1 == "Yes" && BMHDEBUG_INT2 == "Yes") {
                        $this->_debug_output("x", 'ln' . __LINE__ . '<br> aupost-overseas  return->quotes<br>', $this->quotes);
                    }
                    return $this->quotes;
                    // ----  Cache Exit Point ------------------------------ //
        ...
        if (BMHDEBUG_INT1 == "Yes" && BMHDEBUG_INT2 == "Yes") {
            $this->_debug_output("n", 'ln' . __LINE__ . '<br>parcels ** aupost-oseas url called <br>' .
        ...
        if (str_contains($qu, "Please enter a valid Country code")) {  //  2024-04-07 example PS = Palestinian , State not valid from Aust

        if (BMHDEBUG_INT1 == "Yes" && BMHDEBUG_INT2 == "Yes") {
            $this->_debug_output("n", 'ln' . __LINE__ . '<br>' .$dest_country . ' is not a valid Aus Post destination code. </p>', "");
        }

            // display  error message on screen
            echo ('<p class="aupost-debug" ><strong> An error occurred. ' . json_decode($qu, true)['error']['errorMessage'] . ' ' .
             $dest_country . ' is not a valid Aus Post destination code. </strong>');

             // handle invalid destination country and reset
            $this->_invalid_destination_country(json_decode($qu, true)['error']['errorMessage'] ?? $qu, $dcode);
        ...
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes")) {
            $this->_debug_output("n", 'ln' . __LINE__ . ' Server Returned: aupostint <br>', "");
        }

        $jsonint = ($qu == '') ? array() : json_decode($qu, true);  // If we have any results, parse them into an array

        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes")) {
            $this->_debug_output("x", ' ln' . __LINE__ . '>> Server Returned BMHDEBUG_INT1+2 JSON output
                << <br> ', $jsonint);
        }

        // ---- Check for nil returm from Australia Post
        if (empty($jsonint['services'])) {

            if (BMHDEBUG_INT2 == "Yes") {
                $this->_debug_output("n", 'ln' . __LINE__ . ' quote returned is blank - Country not allowed </p>', "");
            }
        ...
        /*  BMH NOTE: The API returns a list of services available for the destination and weight. Loop through this list and match
         *    it to the allowed methods defined in the admin console. If there is a match we then check if any additional options are
         *    required for that method and if so we make additional API calls to get the cost of those options. Finally we add the
         *    method to our quotes array.
         */
        ...
        if (BMHDEBUG_INT1 == "Yes" && BMHDEBUG_INT2 == "Yes") {
           ...
           // ---- normalise: single service returned as object rather than array
            if (isset($jsonint['services']['service']) && !isset($jsonint['services']['service'][0])) {
                $jsonint['services']['service'] = [$jsonint['services']['service']];
            }

            foreach ($jsonint['services']['service'] as $foo => $bar) {

            //foreach ($jsonint as $key => $value) {
            // keep API code for label
            $code = strval(($jsonint['services']['service'][$i]['code'] ));      //  strval


            $code = str_replace("_", " ", $code);                               //
            $code = substr($code, 11);                                          //strip first 11 chars;  keep API code for label

            $id = str_replace("_", "", strval($jsonint['services']['service'][$i]['code'] ?? ''));    //  strval remove underscores from AusPost methods.
            ...
            $cost = (float) ($jsonint['services']['service'][$i]['price'] );

            $description = "PARCEL " . (ucwords(strtolower($code))); // prepend PARCEL to code in sentence case

            if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes")) {
                $this->_debug_output("n", 'ln' . __LINE__ . "n2  ID= $id DESC= $description COST= " . $cost . ' ex', "");
            ...
            // ---- context for the secondary option wrappers ---- //
            $ictx = array(
                'jsonint' => $jsonint,
                'i' => $i,
                'id' => $id,
                ...
                'aus_rate_int' => $aus_rate_int,
                'MINVALUEEXTRACOVER_INT' => $MINVALUEEXTRACOVER_INT
            );

            switch ($id) {
                case "INTPARCELAIROWNPACKAGING":                          // NOTE No tracking; MAX Weight 3.5kg limited countries
                    ...
                    if (in_array("Economy Air Mail", $this->allowed_methods)) {
                        ...
                        $methods[] = array(
                            'id' => "$id",
                            'title' => $description . " [" . $allowed_option . "] "
                                . $details,
                            'cost' => $cost
                        );   // ---- update method
                    }
                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Economy Air Mail Insured +sig", 1, 1, 'INT_SIGNATURE_ON_DELIVERY', 'INT_AIR_EXTRA_COVER', $id . str_replace("_", "", 'INT_AIR_EXTRA_COVER'), self::GATE_DOWNGRADE, self::DUMP_NONE, false, false, '', $ordervalue, $description, $details, $ictx);

                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Economy Air Mail +sig", 1, 0, 'INT_SIGNATURE_ON_DELIVERY', ' ', $id . str_replace("_", "", 'INT_SIGNATURE_ON_DELIVERY'), self::GATE_NONE, self::DUMP_NONE, false, false, '', $ordervalue, $description, $details, $ictx);

                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Economy Air Mail Insured (no sig)", 0, 1, ' ', 'INT_AIR_EXTRA_COVER', $id . str_replace("_", "", 'INT_AIR_EXTRA_COVER'), self::GATE_DOWNGRADE, self::DUMP_VARDUMP, false, false, '', $ordervalue, $description, $details, $ictx);
                ...
                case "INTPARCELSEAOWNPACKAGING":                            // NOTE MIN Weight 2kg limited countries
                    ...
                    if (in_array("Sea Mail", $this->allowed_methods)) {
                        ...
                        $add_int = MODULE_SHIPPING_OVERSEASAUPOST_SEAMAIL_HANDLING;
                        $f = 1;
                    ...
                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Sea Mail Insured +sig", 1, 1, 'INT_SIGNATURE_ON_DELIVERY', 'INT_EXTRA_COVER', $id . str_replace("_", "", 'INT_SIGNATURE_ON_DELIVERY' . 'INT_EXTRA_COVER'), self::GATE_DOWNGRADE, self::DUMP_NONE, true, false, MODULE_SHIPPING_OVERSEASAUPOST_SEAMAIL_HANDLING, $ordervalue, $description, $details, $ictx);

                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Sea Mail +sig", 1, 0, 'INT_SIGNATURE_ON_DELIVERY', 'INT_EXTRA_COVER', $id . str_replace("_", "", 'INT_SIGNATURE_ON_DELIVERY'), self::GATE_NONE, self::DUMP_NONE, true, false, MODULE_SHIPPING_OVERSEASAUPOST_SEAMAIL_HANDLING, $ordervalue, $description, $details, $ictx);

                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Sea Mail Insured (no sig)", 0, 1, '', 'INT_EXTRA_COVER', $id . str_replace("_", "", 'INT_EXTRA_COVER'), self::GATE_NONE, self::DUMP_DEBUGOUTPUT, false, false, '', $ordervalue, $description, $details, $ictx);
                    break;
                ...
                case "INTPARCELSTDOWNPACKAGING":
                    ...
                    if (in_array("Standard Post International", $this->allowed_methods)) {
                        ...
                        $add_int = MODULE_SHIPPING_OVERSEASAUPOST_STANDARD_HANDLING;
                        $f = 1;
                        $code_sig = 0;
                        $code_cover = 0;
                        ...
                        $methods[] = array(
                            'id' => "$id",
                            'title' => $description . " [" . $allowed_option . "] " . //BMH ADDIN
                                $details,
                            'cost' => $cost
                        );   // ---- update method
                    }
                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Standard Post International Insured +sig", 1, 1, 'INT_SIGNATURE_ON_DELIVERY', 'INT_EXTRA_COVER', "INTPARCELSTDOWNPACKAGING" . "INTSIGNATUREONDELIVERY" . "INTEXTRACOVER", self::GATE_SKIP, self::DUMP_NONE, false, false, '', $ordervalue, $description, $details, $ictx);

                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Standard Post International +sig", 1, 0, 'INT_SIGNATURE_ON_DELIVERY', 'INT_EXTRA_COVER', $id . str_replace("_", "", 'INT_SIGNATURE_ON_DELIVERY'), self::GATE_NONE, self::DUMP_NONE, false, false, '', $ordervalue, $description, $details, $ictx);

                    $this->_int_secondary_option($methods, $included_option, $f, $add_int, "Standard Post International Insured (no sig)", 0, 1, '', 'INT_EXTRA_COVER', $id . str_replace("_", "", 'INT_EXTRA_COVER'), self::GATE_SKIP, self::DUMP_DEBUGOUTPUT, false, false, '', $ordervalue, $description, $details, $ictx);
                    break;
                ...
                case "INTPARCELEXPOWNPACKAGING":
                    ...
                    if (in_array("Express Post International (sig inc) + Insured", $this->allowed_methods, $strict = true)) {
                        $included_option = 1;
                        ...
                        $optionservicecode = ($jsonint['services']['service'][$i]['code']);  // get api code for this option
                        ...
                        $result_int_secondary_options = $this->_get_int_secondary_options(
                            $add_int,
                            $allowed_option,
                            $ordervalue,
                            $MINVALUEEXTRACOVER_INT,
                            $dcode,
                            $parcelweight,
                            $optionservicecode,
                            $OPTIONCODE_COVER,
                ...
                case "INTPARCELCOROWNPACKAGING":
                    ...
                    if (in_array("Courier International", $this->allowed_methods)) {
                        ...
                        $add_int = MODULE_SHIPPING_OVERSEASAUPOST_COURIER_HANDLING;
                        $f = 1;
                ...
                case "INTPARCELREGULARPACKAGELARGE":      // garbage collector
                    ...
                    $f = 0;
                    $add_int = 0;
                    // echo "shouldn't be here"; debug do nothing - ignore the code
                    break;
            ...
            if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes")) {
                $this->_debug_output("n", '<br> n2 ln' . __LINE__ . ' $i=' . $i . "</p>", "");
            } // BMH DEBUG
        ...
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes")) {
            $this->_debug_output("x", 'ln' . __LINE__ . '<br> aupost-overseas x2 all done return->quotes<br>', $this->quotes);
        }
    ...
    /**
     * Summary of _get_int_secondary_options
     * @param mixed $add_int
     * @param mixed $allowed_option
     * @param mixed $ordervalue
     * @param mixed $MINVALUEEXTRACOVER_INT
     * @param mixed $dcode
     * @param mixed $parcelweight
     * @param mixed $optionservicecode
     * @param mixed $OPTIONCODE_COVER
     * @param mixed $OPTIONCODE_SIG
     ...
     * @param mixed $aus_rate_int
     * @param mixed $code_sig
     * @param mixed $code_cover
     */
    private function _get_int_secondary_options(
        $add_int,
        $allowed_option,
        $ordervalue,
        $MINVALUEEXTRACOVER_INT,
        ...
        $result_int_secondary_options = array("id" => '', "title" => '', "cost" => 0.0);
        ...
        if ((in_array($allowed_option, $this->allowed_methods))) {
            ...
            if (($code_sig == 1) && ($code_cover == 0)) {
                ...
                if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes")) {
                    $this->_debug_output("n", 'n2 ln' . __LINE__ . ' <b>sig only </b> ' . PARCEL_INT_URL_STRING_CALC .
                ...
                $cost_option = $jsonquote_2['postage_result']['total_cost'];
            }
            //// No sig + extra cover ////
            if (($code_sig == 0) && ($code_cover == 1)) {
                ...
                if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes")) {
                    $this->_debug_output("n", '<br> n2 ln' . __LINE__ . ' <b>ins no sig </b>before get_auspost_api_int' . PARCEL_INT_URL_STRING_CALC .
                ...
                if (isset($jsonquote_2['postage_result']['error'])) {  //
                    //$invalid_option = $jsonquote_2['postage_result']['error'];
            ...
            if (($code_sig == 1) && ($code_cover == 1)) {
                ...
                $jsonquote_2s = ($qu2_sig == '') ? array() : json_decode($qu2_sig, true); // JSON format

                if (isset($jsonquote_2s['postage_result']['error'])) {  // BMH DEBUG
                    //$invalid_option = $jsonquote_2s['postage_result']['error'];
                    // pass back a zero value as not a valid option from Australia Post eg extra cover may require a signature as well
                    $cost = 0;
                    $result_int_secondary_options = array("id" => '', "title" => '', "cost" => $cost);  // invalid result
                    return $result_int_secondary_options;
                ...
                $cost_sig = $jsonquote_2s['postage_result']['total_cost'];   // cost inc sig
                ...
                $jsonquote_2c = ($qu2_cover == '') ? array() : json_decode($qu2_cover, true); // JSON format

                if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes")) {
                    $this->_debug_output("x", 'ln' . __LINE__ . '<strong>>> Server Returned BMHDEBUG1+2 secondary options sig
                        + cover jsonquote_2c << </strong> <br> ', $jsonquote_2c);
                }

                if (isset($jsonquote_2c['postage_result']['error'])) {  // BMH DEBUG

                    $invalid_option = $jsonquote_2c['postage_result']['error'];
                    // pass back a zero value as not a valid option from Australia Post eg extra cover may require a signature as well
                    $cost = 0;
                    $result_int_secondary_options = array("id" => '', "title" => '', "cost" => $cost);  // invalid result
                    return $result_int_secondary_options;
                ...
                $cost_cover = $jsonquote_2c['postage_result']['costs']['cost'][1]['cost'];   // cost for cover
                $cost_option = $cost_option + $cost_cover;

                if (MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes" && BMHDEBUG_INT1 == "Yes" && BMHDEBUG_INT2 == "Yes") {
                    $this->_debug_output("x", '<strong>ln' . __LINE__ . '>> Server Returned BMHDEBUG1+2
                        secondary options sig + cover jsonquote_2c << </strong> <br> ', $jsonquote_2c);
                }

                // build the main quote
                $jsonquote_2 = ($qu2 == '') ? array() : json_decode($qu2, true); // JSON format

                if (isset($jsonquote_2['postage_result']['error'])) {  // BMH DEBUG
                    $invalid_option = $jsonquote_2['postage_result']['error'];
                    // pass back a zero value as not a valid option from Australia Post eg extra cover may require a signature as well
                    $cost = 0;
                    $result_int_secondary_options = array("id" => '', "title" => '', "cost" => $cost);  // invalid result
            ...
            $desc_option = '[' . $desc_option . ']';         // delimit option in square brackets
            $result_int_secondary_options = array(
                "id" => $id_option,
                "title" => $description . ' ' .
                    $desc_option . ' ' . $details,
                "cost" => $cost
    ...
    /**
     * Builds and appends one secondary option ('Insured +sig', '+sig' or 'Insured (no sig)') for the current parcel type.
     * @param array $methods
     * @param mixed $included_option set to 1 when the option is allowed
     * @param mixed $f set to 1 only when $set_handling (handling & cost flags)
     * @param mixed $add_int shipping method handling constant (set only when $set_handling)
     * @param string $allowed_option admin option label e.g. 'Sea Mail Insured +sig'
     * @param int $code_sig signature flag 0/1 passed to the API option call
     * @param int $code_cover extra cover flag 0/1 passed to the API option call
     * @param string $OPTIONCODE_SIG API option code for signature on delivery
     * @param string $OPTIONCODE_COVER API option code for extra cover
     * @param string $id_option option id to append to the method id
     * @param int $gate_mode one of the GATE_* constants
     * @param int $dump_mode one of the DUMP_* constants
     * @param bool $set_handling true to also refresh $add_int and $f
     * @param bool $strict use strict matching against $allowed_methods
     * @param string $handling_const handling constant applied when $set_handling
     * @param mixed $ordervalue order value used for the minimum extra-cover gate
     * @param mixed $description current parcel description
     * @param mixed $details current shipping details
     * @param array $ictx shared per-iteration context array
     */
    private function _int_secondary_option(
        array &$methods,
        &$included_option,
        &$f,
        &$add_int,
        string $allowed_option,
        int $code_sig,
        int $code_cover,
        string $OPTIONCODE_SIG,
        string $OPTIONCODE_COVER,
        string $id_option,
        int $gate_mode,
        int $dump_mode,
        bool $set_handling,
        bool $strict,
        string $handling_const,
        $ordervalue,
        $description,
        $details,
        array $ictx
        ...
        if (!in_array($allowed_option, $this->allowed_methods, $strict)) {
            return;                                             // admin has not selected this option
        }

        $included_option = 1;
        if ($set_handling) {
            $add_int = $handling_const;
            $f = 1;
        }

        switch ($gate_mode) {
            case self::GATE_DOWNGRADE:
                if ($ordervalue < $ictx['MINVALUEEXTRACOVER_INT']) {
                    $code_cover = 0;
                }
                break;
            case self::GATE_SKIP:
                if ($ordervalue < $ictx['MINVALUEEXTRACOVER_INT']) {
                    return;                                     // option not offered at this order value
                }
                break;
        }

        $optionservicecode = $ictx['jsonint']['services']['service'][$ictx['i']]['code'];  // get api code for this option

        $result_int_secondary_options = $this->_get_int_secondary_options(
            $add_int,
            $allowed_option,
            $ordervalue,
            $ictx['MINVALUEEXTRACOVER_INT'],
            $ictx['dcode'],
            $ictx['parcelweight'],
            $optionservicecode,
            $OPTIONCODE_COVER,
        ...
        switch ($dump_mode) {
            case self::DUMP_VARDUMP:
                ...
                break;
            case self::DUMP_DEBUGOUTPUT:
                if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes")) {
                    $this->_debug_output("n", 'ln' . __LINE__ . 'n2 $result_int_secondary_options = ', $result_int_secondary_options);
                } // DEBUG
                break;
        }
    ...
    } // eof function _int_secondary_option //

        //// //  _get_int_secondary_options
    /**
     * Summary of _get_int_error_cost
     * @param mixed $dest_country
     * @param mixed $error_msg_ap_int
     */
    ...
    // ---- force the customer to reselect the shipping destination country when invalid AP country code is detected ----- //
    private function _invalid_destination_country(string $ap_errormessage, string $countrycode)
    {
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes")) {
            $this->_debug_output("n", 'ln' . __LINE__ . 'n2 _invalid_destination_country() Dest country code = ' . $countrycode . ' <br>', "");
        }
        global $messageStack;

        //echo ('<p class="aupost-debug" ><strong> An error occurred. ' . json_decode($qu, true)['error']['errorMessage'] . ' ' . $dcode . ' is not a valid Aus Post destination code. </strong>');

        unset($_SESSION['overseasaupostQuotes'], $_SESSION['overseasaupostParcel']);   // drop cached quotes for the invalid country
        unset($_SESSION['shipping']);                                                  // un-pick the method so it must be reselected
        $messageStack->add_session('checkout_shipping',
            MODULE_SHIPPING_OVERSEASAUPOST_TEXT_ERROR . ': ' . $countrycode . ' (' . $ap_errormessage . ')', 'error');
        $onCheckoutShipping = ((basename($_SERVER['PHP_SELF'] ?? '') == FILENAME_CHECKOUT_SHIPPING . '.php')
            || (($_GET['main_page'] ?? '') == FILENAME_CHECKOUT_SHIPPING));             // only force re-selection during checkout
        if ($onCheckoutShipping) {
    ...
    //auspost API
    /**
     * Summary of get_auspost_api_int
     * @param mixed $url
     * @return bool|string
     */
    function get_auspost_api_int($url)
    {
        ...
        $jsonint = [];
        global $jsonquote2;
        global $dcode, $dest_country;
        global $messageStack;

        global $customer_id;
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes") && (BMHDEBUG_INT3 == "Yes")) {
        ...
        if (str_contains($ret ?? '', "error")) {          // Issue #48 if (str_contains($ret, "error")) {
            $emsg = json_decode($ret, true)['error']['errorMessage'] ?? $ret;
            //$this->_debug_output("n", 'ln' . __LINE__ . ' AUOS Error Occurred $ret= ' . $emsg . ' </p> ', "");
            if (str_contains($emsg, 'Country')) {         // AP does not recognise the destination country code
                $badcountry = $dcode;                     // keep code for the message before resetting
                $dcode = "";                              //reset the code
                $dest_country = "";                       //reset the code
                $this->_invalid_destination_country($emsg, $badcountry);
            }
        }
        ...
        $jsonint = ($ret == '') ? [] : json_decode($ret, true); // If we have any results, parse them into an array
        /*
         if ($jsonint['errorMessage']) {   ++++++++++++++++++++++ zzzzzzzzzzzzzzzz
             $ret = 'Error ' . $ret;
             $this->_log("" . $jsonint['errorMessage'] . " Cust:" . $customer_id); // write to log file
             $cost = "";
             $methods[] = array('id' => $this->code, 'title ' . $jsonint['errorMessage'], 'cost' => $cost); //BMH issue#19
             $this->quotes['methods'] = $methods;   // set it
             return $ret;
         }
         */
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes") && (BMHDEBUG_INT2 == "Yes") && (BMHDEBUG_INT3 == "Yes")) {
            $this->_debug_output("x", 'ln' . __LINE__ . 'x3 >> Server Returned BMHDEBUG_INT1+2 JSON output
                << <br>', $jsonint);
        }
        return $ret;
    }
    ...
    $add = 0;
        ...
        if (MODULE_SHIPPING_OVERSEASAUPOST_HIDE_HANDLING != 'Yes') {
            ...
            if ($add > 0) {                                     // avoid division of zero
                $add_display = $currencies->format($add / $aus_rate_int);
            } else {
                $add_display = $currencies->format(0);
            }

            $details = ' (Inc ' . $add_display . ' P &amp; H';  // Abbreviated for space saving in final quote format
            if ($info > 0) {
    ...
    public function _debug_output($x, $debug_message, $dump)
    {
        switch ($x) {
            ...
            case "x":            // x=[was xml] now json pretty dump
                echo '<p class="aupost-debug ">';
                echo 'x ' . $debug_message;
                echo '<br> <textarea rows="15" cols="200" readonly="readonly">';
                print_r($dump);
                //echo json_encode($dump, JSON_PRETTY_PRINT); // BMH changed to JSON format
                echo "</textarea> </p>";
                break;
