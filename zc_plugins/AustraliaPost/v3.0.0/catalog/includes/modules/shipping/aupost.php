...
/**
 * $Id:   aupost.php,v3.0.0 Apr 2026
 * 2026-04-17 BMH - v3.0.0 - Conversion from XML API to JSON API
 */
// BMHDEBUG switches // WARNING DO NOT ENABLE FOR PRODUCTION
define('BMHDEBUG1', 'No');    // No or Yes // BMH 2nd level debug
define('BMH_P_DEBUG2', 'No'); // No or Yes // BMH 3nd level debug to display all returned json data from Aus Post
define('BMH_P_DEBUG3', 'No'); // No or Yes // BMH 4th level debug to display raw curl  returned json data from Aus Post
//
...
class aupost extends base
{
    ...
    public $ordervalue_ori;     // original value of order before any adjustments
    ...
    public function __construct()
    {
        ...
        $this->error_msg_ap = '';
        //$this->frompcode =   defined('MODULE_SHIPPING_AUPOST_SPCODE'); // *** CHECKED - this is set in the admin panel and is not a constant. It is retrieved as MODULE_SHIPPING_AUPOST_SPCODE
        ...
        $this->tax_class = defined('MODULE_SHIPPING_AUPOST_TAX_CLASS') && MODULE_SHIPPING_AUPOST_TAX_CLASS;   // $this->tax_basis = 'Shipping' ;  // It will always work this way, regardless of any global settings
    ...
    public function quote($method = '')
    {
        ...
        $maxcover = 0;
        global $maxcoverexceeded;  // initialise max cover exceeded flag to false
        ...
        // PARCELS - values
        // Maximums - parcels
        $MAXWEIGHT_P = 22;     // Aus Post max weight 22kg since 2021-10
        ...
        if (($letterheight) <= $MAXHEIGHT_L) {
            ...
            // L_DEBUG2 display the letter values ';
                    ...
                    case "AUSLETTEREXPRESSLARGE":
                        if ((in_array("Aust Express", $this->allowed_methods_l))) {
                            ...
                            (
                                ...
                                // DEBUG mask for testing // setting value forces extra cover on request at Post office
        ...
        if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
            //$this->_debug_output("n","<table class='aupost-debug'><tr><td><b>n2 auPost - Server Returned BMH_P_DEBUG2 ln " . __LINE__ . "</b><br>" . $qu . "</td></tr></table> ","");
            $this->_debug_output("x", "<b>n2 auPost - Server Returned BMH_P_DEBUG2 ln" . __LINE__ . ":</b><br> ", $json);
        }

        // Check for returned quote is really an error message
        /*    if(str_starts_with($qu, "{" )) {
                if ($myerrorarray['status'] == "Failed") {
                    echo '<br> ln'. __LINE__ . ': Australia Post connection ' . $myerrorarray['status'] . '. Please report error to site owner';
                    $this->_log("". implode(',', $myerrorarray) .   " Cust:". $customer_id, "$qu ={"); // BMH
                return $this->quotes;
                }
            }
         */
        // BMH reinstate error trap later
        ...
        if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {  //json output
            $this->_debug_output("x", " d2 <strong> Server Returned BMHDEBUG1+2 ln" . __LINE__ . " <br> </strong>  ", $json);
        }

        $maxcover = ($json['services']['service'][0]['max_extra_cover']);  // BMH cast to int

        if ((int) $ordervalue_ori > (int) $maxcover) {  // BMH cast to int
            $maxcoverexceeded = True; // set flag to indicate order value exceeds max extra cover for express letter
        ...
        if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
            $this->_debug_output("n", " n2 ln" . __LINE__ . " Max extra cover available = " . $maxcover, "");
            $this->_debug_output("n", " n2 ln" . __LINE__ . " Max cover exceed flag = " . $maxcoverexceeded, "");
        }
        ...
        $i = 0;  // counter
        if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
            $this->_debug_output("d", " d2 ln" . __LINE__ . ' $this->allowed_methods = <br> ', $this->allowed_methods); // BMH ** DEBUG
        }
        if (BMH_MIN_ORDER_VALUE_DEBUG == "Yes") {
            $ordervalue = $MINVALUEEXTRACOVER + 1;
        }                          //  ** DEBUG to force extra cover value FOR TESTING ONLY; auto cover to $100
        ...
        foreach ($json['services']['service'] as $foo => $bar) {
            ...
            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                $this->_debug_output("n", " n2 ln" . __LINE__ . " ID= $id  DESC= $description COST= $cost inc", "");
            ...
            switch ($id) {
                ...
                case "AUSPARCELREGULARSATCHELEXTRASMALL":      // fall through and treat as one block // BMH v2.5.8
                    ...
                    if (in_array("Prepaid Satchel", $this->allowed_methods, $strict = true)) {
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Prepaid Satchel $maxcoverexceeded is True reset', ""); // 
                            }
                        ...
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", " n2 ln" . __LINE__ . " allowed option = prepaid satchel", "");
                        }
                        ...
                        if ((($cost > 0) && ($f == 1))) { //
                            ...
                            if (MODULE_SHIPPING_AUPOST_CORE_WEIGHT == "Yes")
                                $cost = ($cost * $this->shipping_num_boxes);
                    ...
                    if (in_array("Prepaid Satchel Insured +sig", $this->allowed_methods)) {
                        ...
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Prepaid Satchel Insured +sig $maxcoverexceeded =True NO CHANGE', ""); // ** DEBUG
                            }
                    ...
                    if (in_array("Prepaid Satchel +sig", $this->allowed_methods)) {
                        ...
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Prepaid Satchel +sig $maxcoverexceeded is True $ordervalue reset', ""); //BMH ** DEBUG
                            }
                            $ordervalue = $maxcover - 1;
                        } else {
                            $ordervalue = $ordervalue_ori;
                    ...
                    if (in_array("Prepaid Satchel Insured (no sig)", $this->allowed_methods)) {
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . " Prepaid Satchel Insured (no sig) maxcover exceed= " . ($maxcoverexceeded ? 'true' : 'false') . " order value = $ordervalue break ", ""); //BMH ** DEBUG
                            }
                            break;
                        } else {
                            $ordervalue = $ordervalue_ori;
                        ...
                        if ($ordervalue > $MINVALUEEXTRACOVER) {
                            ...
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("d", ' d2 ln' . __LINE__ . ' $result_secondary_options = ', $result_secondary_options); //BMH ** DEBUG
                            }
                ...
                case "AUSPARCELEXPRESSSATCHELEXTRASMALL":      // fall through and treat as one block
                    ...
                    if ((in_array("Prepaid Express Satchel", $this->allowed_methods))) {
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", " ln1043 n2 allowed option = parcel express satchel", "");
                        }

                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Prepaid Express Satchel $maxcoverexceeded $ordervalue reset', ""); //BMH ** DEBUG
                            }
                            $ordervalue = $maxcover - 1;
                        } else {
                            $ordervalue = $ordervalue_ori;
                    ...
                    if (in_array("Prepaid Express Satchel Insured +sig", $this->allowed_methods)) {
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", " n2 ln" . __LINE__ . " allowed option = parcel express satchel ins+sig", "");
                            $this->_debug_output("n", " n2 ln" . __LINE__ . " ordervalue = ", $ordervalue);
                        }
                    ...
                    if (in_array("Prepaid Express Satchel +sig", $this->allowed_methods)) {
                        ...
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", '  n2 ln' . __LINE__ . ' Prepaid Express Satchel +sig $maxcoverexceeded $ordervalue break', ""); //BMH ** DEBUG
                            }
                            break;
                        } else {
                            $ordervalue = $ordervalue_ori;
                    ...
                    if (in_array("Prepaid Express Satchel Insured (no sig)", $this->allowed_methods)) {
                        ...
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Prepaid Express Satchel Insured (no sig) $maxcoverexceeded break', ""); //BMH ** DEBUG
                            }
                            break;
                        } else {
                            $ordervalue = $ordervalue_ori;
                ...
                case "AUSPARCELREGULAR":                       // normal mail - own packaging
                    if (in_array("Regular Parcel", $this->allowed_methods, $strict = true)) {
                        ...
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", ' n2 ln' . __LINE__ . ' allowed option = parcel regular', "");
                        }

                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Regular Parcel $maxcoverexceeded reset', ""); //BMH ** DEBUG
                            }
                            $ordervalue = $maxcover - 1;
                        } else {
                            $ordervalue = $ordervalue_ori;
                    ...
                    if (in_array("Regular Parcel Insured +sig", $this->allowed_methods)) {
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", ' ln' . __LINE__ . ' n2 allowed option = parcel regular ins + sig', "");
                        }

                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Regular Parcel Insured +sig $maxcoverexceeded NO CHANGE', ""); //BMH ** DEBUG
                            }
                            $ordervalue = $ordervalue_ori;
                        } // reset if max extra cover exceeded
                    ...
                    if (in_array("Regular Parcel +sig", $this->allowed_methods)) {
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", ' n2 ln' . __LINE__ . ' n2 allowed option = parcel regular + sig', "");
                        }

                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Regular Parcel +sig $maxcoverexceeded reset', ""); //BMH ** DEBUG
                            }
                            $ordervalue = $maxcover - 1;
                        } else {
                            $ordervalue = $ordervalue_ori;
                    ...
                    if (in_array("Regular Parcel Insured (no sig)", $this->allowed_methods)) {
                        ...
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Regular Parcel Insured (no sig) $maxcoverexceeded break', ""); //BMH ** DEBUG
                            }
                            break;
                        } else {
                            $ordervalue = $ordervalue_ori;
                        ...
                        if ($ordervalue > $MINVALUEEXTRACOVER) {
                            ...
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("d", ' ln' . __LINE__ . ' d2 $result_secondary_options = ', $result_secondary_options);
                            }
                            if (strlen($id) > 1) {
                                $methods[] = $result_secondary_options;
                            }
                ...
                case "AUSPARCELEXPRESS":              // express mail - own packaging
                    if (in_array("Express Parcel", $this->allowed_methods, $strict = true)) {
                        ...
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", ' ln' . __LINE__ . ' n2 allowed option = parcel express', "");
                        }
                        $optioncode = "";
                        $optionservicecode = "";
                        $suboptioncode = "";
                    ...
                    if (in_array("Express Parcel Insured +sig", $this->allowed_methods, $strict = true)) {
                        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                            $this->_debug_output("n", ' ln' . __LINE__ . ' n2 allowed option = parcel express ins + sig', "");
                        }
                        if ($ordervalue > $MINVALUEEXTRACOVER) {
                            $optioncode = 'AUS_SERVICE_OPTION_SIGNATURE_ON_DELIVERY';
                            $add = MODULE_SHIPPING_AUPOST_EXP_HANDLING;
                    ...
                    if (in_array("Express Parcel +sig", $this->allowed_methods, $strict = true)) {
                        ...
                        if ($maxcoverexceeded === True) {
                            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                                $this->_debug_output("n", ' n2 ln' . __LINE__ . ' Prepaid Express Satchel Insured (no sig) maxcoverexceeded=' . $maxcoverexceeded, ""); //BMH ** DEBUG
                            }
                            $ordervalue = $maxcover - 1;
                        } else {
                            $ordervalue = $ordervalue_ori;
    ...
    private function _get_secondary_options(
        ...
        if ((in_array($allowed_option, $this->allowed_methods))) {
            ...
            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                $this->_debug_output("n", ' <br>n2 ln' . __LINE__ . ' allowed option = ' . $allowed_option . ' url= ' . PARCEL_URL_STRING_CALC . $frompcode .
            ...
            if ((MODULE_SHIPPING_AUPOST_DEBUG == "Yes") && (BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes")) {
                $this->_debug_output("x", '<br> ln' . __LINE__ . ' d2  $allowed_option = ' . $allowed_option . ' <br> ' . 'Server Returned BMHDEBUG1+2 ln' . __LINE__ . 'options<< <br> <textarea>', $jsonquote_2);
            }
    ...
    function get_auspost_api($url)
    {
        if ((BMHDEBUG1 == 'Yes') && (BMH_P_DEBUG2 == 'Yes') && (BMH_P_DEBUG3 == 'Yes')) {
            echo '<br> ln' . __LINE__ . ' x3 get_auspost_api called';
        }
        ...
        if ((BMHDEBUG1 == 'Yes') && (BMH_P_DEBUG2 == 'Yes') && (BMH_P_DEBUG3 == 'Yes')) {
            echo '<br> ln' . __LINE__ . ' x3 get_auspost_api $url= ' . $url;
            echo '<br> ln' . __LINE__ . ' x3 $aupost_url_apiKey= ' . $aupost_url_apiKey;
        }
        ...
        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes") && (BMH_P_DEBUG3 == "Yes")) {
            $this->_debug_output("x", 'ln' . __LINE__ . ' x2 get_auspost_api curl $ret= ', $ret); // will display empty box
        }
        ...
        if ((BMHDEBUG1 == "Yes") && (BMH_P_DEBUG2 == "Yes") && (BMH_P_DEBUG3 == "Yes")) {
            $this->_debug_output("x", 'ln' . __LINE__ . ' x2  $ret= <br>', $ret); // will display empty box
        }
        ...
        if (property_exists($json, 'errorMessage') && !empty($json->errorMessage)) {
            ...
            $this->_log("" . $json->errorMessage . " Cust:" . $customer_id); // BMH write to log file
            $cost = "";

            $methods[] = array('id' => $this->code, 'title ' . $json->errorMessage, 'cost' => $cost); //BMH issue#19
            $this->quotes['methods'] = $methods;   // set it
            return $ret;
        }
    ...
    public function _debug_output($x, $debug_message, $dump)
    {
        switch ($x) {
            case "x":            // x=[was xml] now json pretty dump
                //echo '<p class="aupost-debug"><textarea>';
                echo 'x ' . $debug_message;
                echo '<br> ' .'<textarea rows="15" cols="220" readonly="readonly">';
                //print_r($dump);
