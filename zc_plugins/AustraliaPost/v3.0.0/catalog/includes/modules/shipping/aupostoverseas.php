...
/*
* $Id:   overseasaupost.php, v3.0.0 Apr 2026
*  v3.0.0 2026-04-17 BMH - v3.0.0 - Conversion from XML API to JSON API
*/
...
class aupostoverseas extends base
{
    ...
    public function quote($method = '')
    {
        ...
        if (str_contains($qu, "Please enter a valid Country code")) {  // BMH 2024-04-07 example PS = Palestinian , State not valid from Aust
            echo ('<p class=\"aupost-debug\" ><strong> An error occurred. ' . $qu . ' ' . $dcode . ' is not a valid Aus Post destination code. </strong>');
        ...
        if ((MODULE_SHIPPING_OVERSEASAUPOST_DEBUG == "Yes") && (BMHDEBUG_INT1 == "Yes")) {
            $this->_debug_output("x", '<strong> ln' . __LINE__ . '>> Server Returned BMHDEBUG_INT1+2 JSON output
                << <br> </strong> ', $jsonint);
        }
    ...
    function _get_int_secondary_options(
        $add_int,
        $allowed_option,
        $ordervalue,
        $MINVALUEEXTRACOVER_INT,
...
//auspost API
    function get_auspost_api_int($url)
    {
        ...
        if (str_contains($ret, "error")) {
            $this->_debug_output("n", 'ln' . __LINE__ . ' Error Occurred $ret= ' . json_decode($ret, true)['errorMessage'] . ' </p> ', "");
        }
